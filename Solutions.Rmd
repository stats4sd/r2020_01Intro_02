...
The main trick here is to use the `c()` function to replace the $\pm$ operator as this is equivalent to saying "plus one times" or "minus one times", so we can provide a vector of `-1` and `1` to the code we write so that we only have to write one line for the main part of the solution.
